...
@app.route('/fuckCristiane',methods = ['GET','POST'])
def wew():
    if request.method == 'POST':
        if (request.is_json):
            teste = request.get_json(force=False, silent=False, cache=True)
            resp = glpk.solve(teste)            
            return(resp)
        else:
            return 'pls send a JSON post'
    else:
        print("wow do NOT get")
        return 'mande um post,and always remember: fuck Cristiane!'
